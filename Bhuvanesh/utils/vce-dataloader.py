...
class NaryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image, label = self.dataset[idx]
        nary_label = len(self.target_class_idx)
        if(label in self.target_class_idx):
            nary_label = self.target_class_idx.index(label)
        return image, nary_label
...
def getNaryDataLoader(target_class_names = None, path_to_dataset = None, batch_size = 32, trans=None):
    dataset = datasets.ImageFolder(root=path_to_dataset, transform=trans)
    target_class_idx = [dataset.class_to_idx[tgt] for tgt in target_class_names]
    nary_dataset = NaryDataset(dataset, target_class_idx)
    naryDataLoader = DataLoader(nary_dataset, batch_size=batch_size, shuffle = True)
    return naryDataLoader
...
if __name__ == '__main__':
        ...
        path = "/kaggle/input/vce-dataset/training"
        binDL = getBinaryDataLoader(image_size=(224,224),target_class_name="Normal", 
                                    path_to_dataset= path, batch_size= 32, trans = transform)
        visualize_batch(binDL)
        print("Verified binary")
        allDL = getAllDataLoader(image_size=(224,224), path_to_dataset= path, batch_size=32, trans = transform)
        visualize_batch(allDL)
        print("Verified All")
        naryDL = getNaryDataLoader(target_class_names = ["Normal", "Erosion", "Worms"], path_to_dataset = path, batch_size = 32, trans=transform)
        visualize_batch(naryDL)
        print("Verified Nary")