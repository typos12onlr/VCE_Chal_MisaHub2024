...
class CNNBranch(nn.Module):
    def __init__(self, cnnmodel = 'resnet18',out_dim=512):
        super(CNNBranch, self).__init__()
        #Using a pre-trained ResNet as the CNN backbone (can replace with any other CNN)
        model_func = getattr(models, cnnmodel.lower())
        self.cnn = model_func(pretrained=True)
...
class ViTCNNModel(nn.Module):
    def __init__(self, cnnmodel = "resnet18",num_classes=10, feature_dim=512):
        '''
        cnnmodel: Model used for CNN Branch. Can be any variant of RESNET
        num_classes: Final Classification task
        feature_dim: dimension of output features of both ViT and CNN
        '''
        self.feature_dim= feature_dim
        super(ViTCNNModel, self).__init__()
        self.cnn_branch = CNNBranch(cnnmodel = cnnmodel, out_dim = feature_dim)  # CNN branch
...
# %% [code] {"execution":{"iopub.status.busy":"2024-10-21T05:46:26.105347Z","iopub.execute_input":"2024-10-21T05:46:26.105792Z","iopub.status.idle":"2024-10-21T05:46:26.844774Z","shell.execute_reply.started":"2024-10-21T05:46:26.105750Z","shell.execute_reply":"2024-10-21T05:46:26.843671Z"},"jupyter":{"outputs_hidden":false}}
